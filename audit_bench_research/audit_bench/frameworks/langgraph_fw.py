"""
Framework D: LangGraph.

StateGraph with TypedDict state. The differentiator: conditional edge from skeptic back to
senior if skeptic raises objections the senior did not address. This is the loop plain
Python/CrewAI don't natively provide. Max 2 skeptic->senior revisits before forcing partner.

Nodes: read_evidence -> senior -> skeptic -> partner -> independence.
"""
import sys
import json
...
def run_case(case_dir: Path, model: str, logger: ResearchLog) -> dict:
    ...
    def node_senior(state: CaseState) -> dict:
        ...
        user = (
            ...
            + "\n\nEnumerate red flags (each citing evidence). If revisiting, address the skeptic's objections explicitly."
        )
        ...
        return {
            ...
            "transcript": [{"agent": "AuditSenior", "round": state.get("skeptic_revisits", 0), "message": content}]
        }
    ...
    def node_partner(state: CaseState) -> dict:
        ...
        user = (
            ...
            f"Give the final audit opinion.\n{OUTPUT_SCHEMA_INSTRUCTION}"
        )
        ...
        return {
            ...
            "transcript": [{"agent": "EngagementPartner", "round": 1, "message": content}]
        }
    ...
    def node_independence(state: CaseState) -> dict:
        ...
        return {
            ...
            "transcript": [{"agent": "IndependenceChecker", "round": 1, "message": content}]
        }
    ...
    t0 = time.time()
    try:
        final_state = compiled.invoke({"evidence": "", "senior_red_flags": [], "skeptic_objections": [], "skeptic_revisits": 0, "transcript": []})
        dt = time.time() - t0
        logger.log(f"[{FRAMEWORK_NAME}] graph done in {dt:.1f}s revisits={final_state.get('skeptic_revisits',0)}")
