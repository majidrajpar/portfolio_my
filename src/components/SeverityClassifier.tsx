...
import modelDataRaw from '../data/severity-model.json';
import type { FraudMLModel as MLModel, MLPrediction } from '../types';

const modelData = modelDataRaw as MLModel;

// ─── FIELD DEFINITIONS ───────────────────────────────────────────────
type FieldId = 'exposure' | 'likelihood' | 'detection' | 'scope';

interface SeverityFieldOption {
  label: string;
  desc: string;
  value: number;
  text: string;
...
interface SeverityField {
  ...
  label: string;
  hint: string;
  options: SeverityFieldOption[];
}

const FIELDS: SeverityField[] = [
  {
    id: 'exposure',
...
function mlPredict(exposure: number, likelihood: number, detection: number, scope: number): MLPrediction {
...
// ─── STATE TYPE ──────────────────────────────────────────────────────
type SelectionsState = Record<FieldId, number | null>;

// ─── REPORT TEXT BUILDER ─────────────────────────────────────────────
function buildFindingText(selections: SelectionsState, severityConfig: SeverityConfigItem): string {
  const get = (fieldId: FieldId) => {
    const field = FIELDS.find(f => f.id === fieldId);
    if (!field) return '[not selected]';
...
// ─── MAIN COMPONENT ──────────────────────────────────────────────────
const SeverityClassifier: React.FC = () => {
  ...
  const prediction = allSelected
    ? mlPredict(
        selections.exposure as number,
        selections.likelihood as number,
        selections.detection as number,
        selections.scope as number
      )
    : null;

  const severityConfig = prediction ? SEVERITY_CONFIG[prediction.className] : null;
  const findingText = prediction && severityConfig ? buildFindingText(selections, severityConfig) : '';

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(findingText);
    } catch {
      const el = document.getElementById('finding-output-raw') as HTMLTextAreaElement;
  ...
  const handleSelect = (fieldId: FieldId, value: number) => {
