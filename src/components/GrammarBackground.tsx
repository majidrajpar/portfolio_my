...
function renderStats(Plot: any, _el: HTMLElement, w: number, h: number) {
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.dot(data.filter((_: any, _i: number) => _i % 10 === 4), {
        x: 'q', y: 'findings',
...
function renderCapabilities(Plot: any, _el: HTMLElement, w: number, h: number) {
...
function renderDarkGrid(Plot: any, _el: HTMLElement, w: number, h: number) {
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.dot(data.filter((d: any) => d.score >= 16), {
        x: 'l', y: 'i',
        r: (d: any) => 5 + d.score * 0.5,
        fill: '#C9A84C', fillOpacity: 0.3,
        stroke: '#C9A84C', strokeOpacity: 0.15, strokeWidth: 0.5,
      }),
      Plot.dot(data.filter((d: any) => d.score >= 9 && d.score < 16), {
        x: 'l', y: 'i',
        r: (d: any) => 4 + d.score * 0.4,
        fill: '#ffffff', fillOpacity: 0.1,
        stroke: '#ffffff', strokeOpacity: 0.08, strokeWidth: 0.5,
      }),
      Plot.dot(data.filter((d: any) => d.score < 9), {
        x: 'l', y: 'i',
        r: (d: any) => 3 + d.score * 0.3,
...
const RENDERERS: Record<string, (Plot: any, el: HTMLElement, w: number, h: number) => any> = {
  stats: renderStats,
  capabilities: renderCapabilities,
  'dark-grid': renderDarkGrid,
};

export default function GrammarBackground({ variant = 'stats', className = '' }: { variant?: string; className?: string }) {
  const containerRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    const el = containerRef.current as (HTMLDivElement & { _roCleanup?: () => void }) | null;
    if (!el) return;
    ...
    dynamicImport('https://cdn.jsdelivr.net/npm/@observablehq/plot@0.6.17/+esm').then((Plot: any) => {
      if (cancelled) return;
    ...
    return () => {
      cancelled = true;
      if (el && el._roCleanup) { el._roCleanup(); delete el._roCleanup; }
