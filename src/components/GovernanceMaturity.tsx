...
import modelDataRaw from '../data/governance-maturity-model.json';
...
// ─── MATURITY CONFIG ──────────────────────────────────────────────────
export type MaturityBand = 'Initial' | 'Developing' | 'Defined' | 'Managed' | 'Optimising';

interface MaturityConfigItem {
  level: number;
  color: string;
  textClass: string;
  bgClass: string;
...
const MATURITY_CONFIG: Record<string, MaturityConfigItem> = {
  Initial: {
    level: 1,
    color: '#ef4444',
...
// ─── DOMAIN BAND HELPER ───────────────────────────────────────────────
function domainBand(score: number): string {
...
// ─── PROGRESS DOTS ────────────────────────────────────────────────────
function ProgressDots({ domainIdx, answers }: { domainIdx: number; answers: Record<string, number | null> }) {
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────
const GovernanceMaturity: React.FC = () => {
  ...
  const domainAvgs = useMemo(() =>
    DOMAINS.map(d => {
      const vals = d.questions.map((_, qi) => answers[`${d.id}_${qi}`]).filter(v => v !== null) as number[];
  ...
  const handleAnswer = (domainId: string, qIdx: number, value: number) => {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const el = document.getElementById('gov-report-raw') as HTMLTextAreaElement;
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-[10px] text-amber-600 leading-relaxed">
                      <span className="font-black text-amber-700 uppercase tracking-widest">Advisory Note — </span>
                      This tool uses a deterministic model based on general best practices. It does not replace a formal governance audit.
                    </p>
                  </div>
                </div>
                <button
