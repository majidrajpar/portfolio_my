...
import modelDataRaw from '../data/fraud-risk-model.json';
import type { FraudDomain, RiskConfigItem, RiskLevel, FraudMLModel, MLPrediction } from '../types';

const modelData = modelDataRaw as FraudMLModel;

// ─── DOMAIN DEFINITIONS ──────────────────────────────────────────────
const DOMAINS: FraudDomain[] = [
  {
    id: 'opportunity',
    label: 'Opportunity',
...
// ─── RISK CONFIG ─────────────────────────────────────────────────────
const RISK_CONFIG: Record<string, RiskConfigItem> = {
  Low: {
    label: 'LOW',
    bg: 'bg-green-500/10',
...
function domainRiskLevel(score: number): RiskLevel {
...
function mlPredict(inputs: number[], model: FraudMLModel): MLPrediction {
...
// ─── REPORT BUILDER ──────────────────────────────────────────────────
function buildReportText(domainScores: Record<string, number | null>, prediction: MLPrediction, recommendations: string[]): string {
  const lines = [
    ...
    ...DOMAINS.map(d => {
      ...
      const level = score !== null ? domainRiskLevel(score) : 'Unknown';
      return `  ${d.label}: ${score !== null ? score.toFixed(1) : 'N/A'} / 5  [${level}]`;
    }),
    ``,
    `TOP RECOMMENDATIONS:`,
...
// ─── MAIN COMPONENT ──────────────────────────────────────────────────
const FraudRiskCalculator: React.FC = () => {
  ...
  const domainScores = answers.map(
    (domainAnswers) => {
      const filled = domainAnswers.filter(v => v !== null) as number[];
      return filled.length === 5 ? filled.reduce((s, v) => s + v, 0) / 5 : null;
    }
  );

  const mlInputs = domainScores.every(s => s !== null)
    ? [domainScores[0], domainScores[1], domainScores[2], domainScores[3]] as number[]
    : null;
  ...
  if (prediction) {
    ...
    const scored = domainIds.map((id, i) => ({ id, score: domainScores[i] as number }));
  ...
  const radarData = DOMAINS.map((d, i) => ({
    domain: d.label,
    'Your Score': domainScores[i] !== null ? parseFloat((6 - (domainScores[i] as number)).toFixed(2)) : 0,
    'Target': 4.5,
  }));

  // ── Handlers ──────────────────────────────────────────────────────
  const handleSelect = (domainIdx: number, questionIdx: number, value: number) => {
    setAnswers(prev => {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const el = document.getElementById('fraud-report-raw') as HTMLTextAreaElement;
              ...
              <div className="border border-slate-200 overflow-hidden">
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {DOMAINS.map((d, i) => {
                      const score = domainScores[i] as number;
