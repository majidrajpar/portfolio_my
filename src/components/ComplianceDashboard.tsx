...
function avgScore(frameworkId: string, scores: Record<string, Record<string, number>>) {
...
function initScores(frameworkId: string) {
  const fw = FRAMEWORKS.find(f => f.id === frameworkId);
  if (!fw) return {};
  return Object.fromEntries(fw.controls.map(c => [c, 50]));
}

function buildReport(selected: Set<string>, scores: Record<string, Record<string, number>>) {
  ...
  const overallAvg = Math.round(
    selectedFws.reduce((sum, fw) => sum + avgScore(fw.id, scores), 0) / (selectedFws.length || 1)
  );
...
const ComplianceDashboard = () => {
  ...
  const handleSlider = useCallback((frameworkId: string, control: string, value: string) => {
    setScores(prev => ({
      ...prev,
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const el = document.getElementById('compliance-report-raw') as HTMLTextAreaElement;
