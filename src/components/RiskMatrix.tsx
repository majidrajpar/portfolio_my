...
// ─── SCORING ─────────────────────────────────────────────────────────
function getScoreData(likelihood: number, impact: number): { score: number; rating: RiskLevel; color: string } {
...
// (Unused getRatingForScore removed)
...
function toSVGCoords(likelihood: number, impact: number) {
...
// ─── 5×5 GRID ────────────────────────────────────────────────────────
function RiskGrid({ risks }: { risks: RiskItem[] }) {
  const [tooltip, setTooltip] = useState<TooltipState | null>(null);

  // Group risks by coordinate key
  const coordMap: Record<string, RiskItem[]> = {};
  risks.forEach((r) => {
  ...
  return (
    ...
    >
      ...
      {/* ── Risk dots ─────────────────────────────────────────────── */}
      {Object.values(coordMap).map((group) =>
        group.map((risk, idx) => {
...
function ScoreButtons({ label, value, onChange }: ScoreButtonsProps) {
  return (
...
// ─── REPORT BUILDER ───────────────────────────────────────────────────
function buildReport(risks: RiskItem[]): string {
  ...
  const countByRating: Record<RiskLevel, number> = { Critical: 0, High: 0, Medium: 0, Low: 0 };
  sorted.forEach((r) => {
...
const RiskMatrix: React.FC = () => {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const el = document.getElementById('risk-matrix-report-raw') as HTMLTextAreaElement;
