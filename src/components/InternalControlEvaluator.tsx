...
import modelDataRaw from '../data/icfr-control-model.json';
import type { FraudMLModel as MLModel, MLPrediction } from '../types';

const modelData = modelDataRaw as MLModel;
...
const COSO_COMPONENTS: CosoComponent[] = [
  {
    id: 'control_env',
...
const RATING_COLORS: Record<number, { selected: string; unselected: string }> = {
...
// ─── DEFICIENCY CONFIG ────────────────────────────────────────────────
export type DeficiencyBand = 'No Deficiency' | 'Control Deficiency' | 'Significant Deficiency' | 'Material Weakness';

interface DeficiencyConfigItem {
  bg: string;
  border: string;
  text: string;
  bar: string;
...
const DEFICIENCY_CONFIG: Record<DeficiencyBand, DeficiencyConfigItem> = {
  'No Deficiency': {
    bg: 'bg-green-500/10',
...
function scoreToBand(score: number): DeficiencyBand {
...
function bandToTextColor(band: DeficiencyBand): string {
...
// ─── ML INFERENCE ────────────────────────────────────────────────────
function polyFeatures(x: number[], powers: number[][]): number[] {
...
function mlPredict(inputs: (number | null)[], model: MLModel): MLPrediction {
  const validInputs = inputs as number[];
  const features = polyFeatures(validInputs, model.poly_powers);
...
function computeAverages(ratings: Record<string, Record<number, number>>): (number | null)[] {
...
// ─── COPY REPORT BUILDER ──────────────────────────────────────────────
function buildReport(averages: (number | null)[], prediction: MLPrediction): string {
...
// ─── MAIN COMPONENT ──────────────────────────────────────────────────
const InternalControlEvaluator: React.FC = () => {
  ...
  const defConfig = prediction ? DEFICIENCY_CONFIG[prediction.className as DeficiencyBand] : null;

  const handleRate = useCallback((compId: string, pIdx: number, value: number) => {
    setRatings(prev => {
      const updated = {
        ...prev,
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const el = document.getElementById('icfr-report-raw') as HTMLTextAreaElement;
  ...
  const chartData = COSO_COMPONENTS.map((comp, i) => ({
    ...
    value: averages[i] !== null ? parseFloat((averages[i] as number).toFixed(2)) : 0,
            ...
            <div className="bg-white border border-slate-200 p-6 mb-6">
              ...
              <div className="space-y-3">
                {modelData.classes.map((cls) => {
                  const cfg = DEFICIENCY_CONFIG[cls as DeficiencyBand];
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {COSO_COMPONENTS.map((comp, i) => {
                      ...
                      const textColor = avg !== null ? bandToTextColor(band as DeficiencyBand) : 'text-slate-400';
                      return (
              ...
              <div className="bg-white border border-slate-200 mb-6">
                ...
                <div className="divide-y divide-slate-100">
                  {weakComponents.map((comp, idx) => {
                    ...
                    const cfg = DEFICIENCY_CONFIG[band as DeficiencyBand] || {};
                    return (
